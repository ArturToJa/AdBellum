	...
	if (HitDataTable)
	{
		                                          ...
		                                          DrawDebugType, Hit, true /*bIgnoreSelf*/))
		{
			...
			if (bSpawnSound && HitFX->Sound.LoadSynchronous())
			{
				...
				EALSGait ActorGait = IALSMobilityDataInterface::Execute_GetGait(MeshOwner);
				EALSStance ActorStance = IALSMobilityDataInterface::Execute_GetStance(MeshOwner);

				const float FinalVolMult = GetSoundVolumeMultiplier(ActorGait, ActorStance);
