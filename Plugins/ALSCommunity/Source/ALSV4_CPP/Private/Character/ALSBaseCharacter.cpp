	...
	: Super(ObjectInitializer.SetDefaultSubobjectClass<UALSCharacterMovementComponent>(CharacterMovementComponentName))
{
	...
	HeadComponent->SetLeaderPoseComponent(GetMesh());
}
...
void AALSBaseCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AALSBaseCharacter, StationaryRole);
	DOREPLIFETIME(AALSBaseCharacter, ControlledInputInterceptor);
	DOREPLIFETIME(AALSBaseCharacter, bRecenterCamera);
	DOREPLIFETIME(AALSBaseCharacter, bUseStationaryADS);
	DOREPLIFETIME(AALSBaseCharacter, bUseMainCameraTransform);
	DOREPLIFETIME(AALSBaseCharacter, ActiveWeaponActor);
...
FRotator AALSBaseCharacter::GetFirstPersonCameraRotationADS() {
...
void AALSBaseCharacter::Tick(float DeltaTime)
{
	...
	else if (MovementState == EALSMovementState::Stationary) 
	{
		
	}
	else if(MovementState == EALSMovementState::Crawl)
	{
		UpdateCharacterMovement();
		UpdateCrawlRotation(DeltaTime);
		CrawlDoEveryFrame(40.0f,4.0f,false,DeltaTime);
	}
...
void AALSBaseCharacter::Multicast_SetOverlayState_Implementation(const EALSOverlayState NewState, bool bForce)
{
...
{
	Multicast_SetOverlayState(NewState, bForce);
}
...
void AALSBaseCharacter::ForceUpdateCharacterState()
{
	...
	Server_SetOverlayState(OverlayState, true);
...
void AALSBaseCharacter::OnViewModeChanged(const EALSViewMode PreviousViewMode)
{
	...
	else if (ViewMode == EALSViewMode::FirstPerson &&( RotationMode == EALSRotationMode::VelocityDirection || RotationMode == EALSRotationMode::Aiming))
	{
		// If First Person, set the rotation mode to looking direction if currently in the velocity direction mode.
		//SetRotationMode(EALSRotationMode::LookingDirection);
	}
	SetOwnerHeadVisibility((ViewMode == EALSViewMode::FirstPerson) ? false : true);
	if (CameraBehavior)
	{
...
void AALSBaseCharacter::UpdateGroundedRotation(float DeltaTime)
{
	if (MovementAction == EALSMovementAction::None)
	{
		...
		if (bCanUpdateMovingRot && !(MovementState == EALSMovementState::Stationary))
		{
...
void AALSBaseCharacter::UpdateCrawlRotation(float DeltaTime)
{
	const bool CanUpdateRotation = (DesiredRotationMode == EALSRotationMode::Aiming) && (OverlayState == EALSOverlayState::RPG || OverlayState == EALSOverlayState::Default || OverlayState == EALSOverlayState::PistolTwoHanded || OverlayState == EALSOverlayState::Rifle);

	if (true) 
	{
		if (MovementAction == EALSMovementAction::None) 
		{
			if ((IsMoving() && HasMovementInput()) || GetSpeed() > 150.0f)
			{
				if (RotationMode == EALSRotationMode::VelocityDirection || RotationMode == EALSRotationMode::LookingDirection) 
				{
					SmoothCharacterRotation(FRotator(0.0f, LastVelocityRotation.Yaw,0.0f),700.0f, CalculateGroundedRotationRate() * 0.9f, DeltaTime);
				}
				else if (RotationMode == EALSRotationMode::Aiming) 
				{
			...
			else //not moving 
			{
				...
				if (UKismetMathLibrary::Abs(GetAnimCurveValue("RotationAmount")) > 0.001f) 
				{
					float DeltaRotationZ = GetAnimCurveValue("RotationAmount") * (DeltaTime / (1.0f / 30.0f));
					AddActorWorldRotation(FRotator(0.0f,DeltaRotationZ, 0.0f));
					TargetRotation = GetActorRotation();
				}
				else
				{
					if (UKismetMathLibrary::Abs(MoveForwardAxisValue) + UKismetMathLibrary::Abs(MoveRightAxisValue) > 0.0f)
					{
						SmoothCharacterRotation(FRotator(0.0f, LastVelocityRotation.Yaw,0.0f), 700.0f, CalculateGroundedRotationRate() * 0.7f, DeltaTime);
					}
				}
			}
		}
...
void AALSBaseCharacter::CrawlDoEveryFrame(float TargetCapsuleSize, float CapsuleInterpSpeed, bool OnlyCapsuleUpdate, float DeltaTime)
{
	//Update capsule size and mesh relative position
	if (GetCapsuleComponent()->GetUnscaledCapsuleHalfHeight() != TargetCapsuleSize) 
	{
		float TargetCapsuleHalfHeight = FMath::FInterpTo(GetCapsuleComponent()->GetUnscaledCapsuleHalfHeight(),TargetCapsuleSize,DeltaTime,CapsuleInterpSpeed);
		GetCapsuleComponent()->SetCapsuleHalfHeight(TargetCapsuleHalfHeight, true);
		GetMesh()->SetRelativeLocation(FVector(0.0f,0.0f, GetCapsuleComponent()->GetUnscaledCapsuleHalfHeight()));
		if (OnlyCapsuleUpdate) return;
	}
	//stop movement when detected obstacle


}

float AALSBaseCharacter::CalculateUpGroundAngle(FVector Axis, float Scale) 
{
	FFindFloorResult OutFloorResult;
	GetCharacterMovement()->FindFloor(GetCapsuleComponent()->GetComponentLocation() + Axis * Scale, OutFloorResult, true);
	
	float ImpactPointCalculation = (GetCapsuleBaseLocation(0.0f).Z - OutFloorResult.HitResult.ImpactPoint.Z > 0.0f) ? 1.0f : -1.0f;

	//Normal based calculations
	FRotator NormalRotator = UKismetMathLibrary::MakeRotFromX(OutFloorResult.HitResult.Normal);
	FVector NormalRotatorVector = UKismetMathLibrary::GetForwardVector(NormalRotator);

	float NormalCalculation = FMath::Abs(UKismetMathLibrary::Dot_VectorVector(NormalRotatorVector, GetActorForwardVector()));
	return ImpactPointCalculation * NormalCalculation;
}

void AALSBaseCharacter::CollisionDetectTraceStartEnd(FVector& Start, FVector& End, FVector& AxisValue)
{
	Start = GetCapsuleBaseLocation(-30.0f);
	//(condition) ? true-clause : false-clause
	float MovementMultiplier = (FMath::Abs(MoveForwardAxisValue * MoveRightAxisValue) > 0.0f) ? 50.0f : 60.0f;

	FVector End_SecondSum = (GetActorForwardVector() * MoveForwardAxisValue) * MovementMultiplier;
	FVector End_ThirdSum = (GetActorRightVector() * MoveRightAxisValue) * MovementMultiplier * 0.7f;

	End = Start + End_SecondSum + End_ThirdSum;

	AxisValue = End_SecondSum + End_ThirdSum;
}

void AALSBaseCharacter::GroundDetectTraceStartEnd(FVector& Start, FVector& End, FVector& AxisValue) 
{
	float MovementMultiplier = (FMath::Abs(MoveForwardAxisValue * MoveRightAxisValue) > 0.0f) ? 80.0f : 100.0f;
	FVector ForwardMovementVector = (GetActorForwardVector() * MoveForwardAxisValue) * MovementMultiplier;
	FVector RightMovementVector = (GetActorRightVector() * MoveRightAxisValue) * MovementMultiplier * 0.7f;

	FVector Sum3Vector = GetCapsuleBaseLocation(-5.0f) + ForwardMovementVector + RightMovementVector;

	Start = Sum3Vector + FVector(0.0f, 0.0f, 25.0f);
	End = Sum3Vector + FVector(0.0f, 0.0f, -30.0f);
	AxisValue = ForwardMovementVector + RightMovementVector;
}
...
void AALSBaseCharacter::ForwardMovementAction_Implementation(float Value)
{
	...
	if (MovementState == EALSMovementState::Stationary) {
		//Server_ForwardMovementAction(Value);
		if (ControlledInputInterceptor)
		{
			if (StationaryRole == EALSStationaryRole::Driver) {
				IALSInputInterface::Execute_ForwardMovementAction(ControlledInputInterceptor, Value);
			}
			else if (StationaryRole == EALSStationaryRole::Gunner || StationaryRole == EALSStationaryRole::Gunner_Standing)
			{
				IALSInputInterface::Execute_ForwardMovementActionOrder(ControlledInputInterceptor, Value);
			}
		}
	}
	else if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir || MovementState == EALSMovementState::Crawl)
	{
...
void AALSBaseCharacter::Server_ForwardMovementAction_Implementation(float Value)
{
	//if (ControlledInputInterceptor)
	//{
	//	if (StationaryRole == EALSStationaryRole::Driver) {
...
void AALSBaseCharacter::ForwardMovementActionCompleted_Implementation()
{
	//Server_ForwardMovementActionCompleted();
	MoveForwardAxisValue = 0.0f;
	if (ControlledInputInterceptor) {
		if (StationaryRole == EALSStationaryRole::Driver)
		{
			IALSInputInterface::Execute_ForwardMovementActionCompleted(ControlledInputInterceptor);
		}
		else if (StationaryRole == EALSStationaryRole::Gunner || StationaryRole == EALSStationaryRole::Gunner_Standing)
		{
			IALSInputInterface::Execute_ForwardMovementActionOrderCompleted(ControlledInputInterceptor);
		}
	}
}

void AALSBaseCharacter::Server_ForwardMovementActionCompleted_Implementation()
{
	//if (ControlledInputInterceptor) {
	//	if (StationaryRole == EALSStationaryRole::Driver)
	//	{
...
void AALSBaseCharacter::RightMovementAction_Implementation(float Value)
{
	...
	if (MovementState == EALSMovementState::Stationary)
	{
		//Server_RightMovementAction(Value);
		if (ControlledInputInterceptor)
		{
			if (StationaryRole == EALSStationaryRole::Driver)
			{
				IALSInputInterface::Execute_RightMovementAction(ControlledInputInterceptor, Value);
			}
			else if (StationaryRole == EALSStationaryRole::Gunner || StationaryRole == EALSStationaryRole::Gunner_Standing)
			{
				IALSInputInterface::Execute_RightMovementActionOrder(ControlledInputInterceptor, Value);
			}
		}
	}
	else if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir)
	{
		// Default camera relative movement behavior
		const FRotator DirRotator(0.0f, AimingRotation.Yaw, 0.0f);
...
void AALSBaseCharacter::Server_RightMovementAction_Implementation(float Value)
{
	//if (ControlledInputInterceptor)
	//{
	//	if (StationaryRole == EALSStationaryRole::Driver)
...
void AALSBaseCharacter::RightMovementActionCompleted_Implementation()
{
	//Server_RightMovementActionCompleted();
	MoveRightAxisValue = 0.0f;
	if (ControlledInputInterceptor) {
		if (StationaryRole == EALSStationaryRole::Driver)
		{
			IALSInputInterface::Execute_RightMovementActionCompleted(ControlledInputInterceptor);
		}
		else if (StationaryRole == EALSStationaryRole::Gunner || StationaryRole == EALSStationaryRole::Gunner_Standing)
		{
			IALSInputInterface::Execute_RightMovementActionOrderCompleted(ControlledInputInterceptor);
		}
	}
}

void AALSBaseCharacter::Server_RightMovementActionCompleted_Implementation()
{
	//if (ControlledInputInterceptor) {
	//	if (StationaryRole == EALSStationaryRole::Driver)
	//	{
...
{
	if ((StationaryRole == EALSStationaryRole::Gunner_Standing && bRecenterCamera) || StationaryRole == EALSStationaryRole::Gunner)
	{
		if (StationaryRole == EALSStationaryRole::Gunner_Standing && bRecenterCamera)
		{
			AddControllerPitchInput(LookUpDownRate * Value);
			if (RotationMode == EALSRotationMode::Aiming) 
			{
				Server_CameraUpAction(LookUpDownRate * Value);
			}
		}
		else if (StationaryRole == EALSStationaryRole::Gunner)
		{
			Server_CameraUpAction(Value);
		}
	}
	else {
		if (GetMovementState() == EALSMovementState::Stationary) {
			bRecenterCamera = false;
		}
		AddControllerPitchInput(LookUpDownRate * Value);
	}
}

void AALSBaseCharacter::Server_CameraUpAction_Implementation(float Value)
{
	//IALSInputInterface::Execute_CameraUpAction(ControlledInputInterceptor, Value);
	if (ControlledInputInterceptor) 
	{
		IALSInputInterface::Execute_GunnerCameraUpAction(ControlledInputInterceptor, Value);
	}
}
...
void AALSBaseCharacter::CameraRightAction_Implementation(float Value)
{
	if ((StationaryRole == EALSStationaryRole::Gunner_Standing && bRecenterCamera) || StationaryRole == EALSStationaryRole::Gunner)
	{
		if (StationaryRole == EALSStationaryRole::Gunner_Standing && bRecenterCamera)
		{
			...
			//IALSInputInterface::Execute_CameraRightAction(ControlledInputInterceptor, LookLeftRightRate * Value);
			Server_CameraRightAction(LookLeftRightRate * Value);
		}
		else if (StationaryRole == EALSStationaryRole::Gunner)
		{ // && bBlockCameraRotation
			//IALSInputInterface::Execute_CameraRightAction(ControlledInputInterceptor, Value);
			Server_CameraRightAction(Value);
		}
	}
	else {
		if (GetMovementState() == EALSMovementState::Stationary) {
...
void AALSBaseCharacter::Server_CameraRightAction_Implementation(float Value)
{
	if (ControlledInputInterceptor)
	{
		IALSInputInterface::Execute_GunnerCameraRightAction(ControlledInputInterceptor, Value);
	}
}


void AALSBaseCharacter::CameraActionCompleted_Implementation() {

	if (GetMovementState() == EALSMovementState::Stationary) {
		GetWorldTimerManager().SetTimer(CameraActionCompletedTimer, this,
			&AALSBaseCharacter::EnableRecenterCamera, 1.5f, false);
	}
}
...
{
	if (MovementState == EALSMovementState::Stationary) {
		//Server_JumpAction(bValue);
		if (ControlledInputInterceptor)
		{
			IALSInputInterface::Execute_JumpAction(ControlledInputInterceptor, bValue);
		}
	}
	else if (bValue)
	{
...
void AALSBaseCharacter::Server_JumpAction_Implementation(bool bValue)
{
	//if (ControlledInputInterceptor)
	//{
	//	IALSInputInterface::Execute_JumpAction(ControlledInputInterceptor, bValue);
...
void AALSBaseCharacter::StanceAction_Implementation()
{
	...
	if (LastStanceInputTime - PrevStanceInputTime <= RollDoubleTapTimeout)
	{
		...
		//}
		SetDesiredStance(EALSStance::Proning);
		return;
	}
	
	//if (LastStanceInputTime - PrevStanceInputTime <= StanceHoldTimeout) 
	//{
	//	SetDesiredStance(EALSStance::Proning);
	//	return;
	//}
...
void AALSBaseCharacter::ScrollAction_Implementation(bool Value) 
{
	if (StationaryRole == EALSStationaryRole::Gunner) 
	{
		//scroll up
		if(!Value)
		{
	...
	else if (ALSDebugComponent)
	{
		ALSDebugComponent->OverlayMenuCycle(Value);
	}
}


void AALSBaseCharacter::EnableRecenterCamera() {
	bRecenterCamera = true;
}

void AALSBaseCharacter::Client_SetUseStationaryADS_Implementation(bool Value)
{
	bUseStationaryADS = Value;
}
...
void AALSBaseCharacter::OnRep_MovementState()
{
	OnMovementStateChanged(EALSMovementState::None);
}

bool AALSBaseCharacter::GetRecenterCamera() {
	return bRecenterCamera;
}

void AALSBaseCharacter::Client_SetRecenterCamera_Implementation(bool value) {
	bRecenterCamera = value;
}

FRotator AALSBaseCharacter::GetStationaryTargetCameraRotation() {
	return StationaryTargetCameraRotation;
}

void AALSBaseCharacter::Server_SitCharacterDown_Implementation(USceneComponent* SeatComponent)
{
	Server_SetOverlayState(EALSOverlayState::Default, false);
	GetCharacterMovement()->bIgnoreClientMovementErrorChecksAndCorrection = true;
	FTimerDelegate AttachmentDelegate = FTimerDelegate::CreateUObject(this, &AALSBaseCharacter::AttachCharacter, SeatComponent);

	GetWorldTimerManager().SetTimer(CharacterAttachmentTimer,
		AttachmentDelegate, 1.0f, false);

	Multicast_SitCharacterDown(SeatComponent);
}

void AALSBaseCharacter::Multicast_SitCharacterDown_Implementation(USceneComponent* SeatComponent)
{
	bRecenterCamera = true;
...
void AALSBaseCharacter::AttachCharacter(USceneComponent* SeatComponent)
{
	//bRecenterCamera = true;
	GetCharacterMovement()->Deactivate();
	GetCharacterMovement()->SetMovementMode(EMovementMode::MOVE_None,0);
	//GetController()->SetControlRotation(GetCapsuleComponent()->GetComponentRotation() + FRotator(0.0f, 90.0f, 0.0f));
	GetMesh()->SetAnimInstanceClass(StationaryAnimClass);
	GetWorldTimerManager().SetTimer(CameraActionCompletedTimer, this,
		&AALSBaseCharacter::EnableRecenterCamera, 0.15f, false);
	Multicast_AttachCharacter(SeatComponent);
}

void AALSBaseCharacter::Multicast_AttachCharacter_Implementation(USceneComponent* SeatComponent)
{
	AttachToComponent(SeatComponent, FAttachmentTransformRules(EAttachmentRule::SnapToTarget, EAttachmentRule::SnapToTarget, EAttachmentRule::KeepWorld, true));
	bRecenterCamera = true;
	GetMesh()->SetAnimInstanceClass(StationaryAnimClass);
	GetCharacterMovement()->Deactivate();
	GetCharacterMovement()->SetMovementMode(EMovementMode::MOVE_None, 0);
	SetMovementState(EALSMovementState::Stationary);
	GetCapsuleComponent()->AddRelativeLocation(FVector(0.0f, 0.0f, 90.0f));
	GetCapsuleComponent()->AddRelativeRotation(FRotator(0.0f, 90.0f, 0.0f));
}

void AALSBaseCharacter::Server_StandCharacterUp_Implementation(FVector ExitLocation) 
{
	HandleExitControlledInterceptor();
	GetCharacterMovement()->bIgnoreClientMovementErrorChecksAndCorrection = false;
	//GetController()->SetControlRotation(FRotator(GetController()->GetControlRotation().Pitch, GetController()->GetControlRotation().Yaw, 0.0f));
	GetController()->SetControlRotation(FRotator(0.0f, 0.0f, 0.0f));
	StationaryRole = EALSStationaryRole::None;
	MultiCast_StandCharacterUp(ExitLocation);
}

void AALSBaseCharacter::MultiCast_StandCharacterUp_Implementation(FVector ExitLocation)
{
	DetachFromActor(FDetachmentTransformRules(EDetachmentRule::KeepWorld,
		EDetachmentRule::KeepWorld,
		EDetachmentRule::KeepRelative, false));
	GetCapsuleComponent()->SetWorldLocation(ExitLocation);
	GetCharacterMovement()->Activate();
	GetCharacterMovement()->SetMovementMode(EMovementMode::MOVE_Walking, 0);
	GetMesh()->SetAnimInstanceClass(ALSBaseAnimClass);
	SetMovementState(EALSMovementState::Grounded);
	GetCapsuleComponent()->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
	GetCapsuleComponent()->SetCollisionResponseToChannel(ECollisionChannel::ECC_Vehicle, ECollisionResponse::ECR_Block);
}

void AALSBaseCharacter::HandleExitControlledInterceptor() {
	if (StationaryRole == EALSStationaryRole::Driver) 
	{
		RightMovementActionCompleted_Implementation();
		ForwardMovementActionCompleted_Implementation();
	}
	else if (StationaryRole == EALSStationaryRole::Gunner_Standing)
	{
		AimActionCompleted_Implementation();
	}
	ControlledInputInterceptor = nullptr;
}

bool AALSBaseCharacter::GetUseStationaryADS() 
{
	return bUseStationaryADS;
}

void AALSBaseCharacter::Client_SetUseMainCameraTransform_Implementation(bool value)
{
	bUseMainCameraTransform = value;
}


bool AALSBaseCharacter::GetUseMainCameraTransform()
{
	return bUseMainCameraTransform;
}


void AALSBaseCharacter::NotifyOpenHud_Implementation(bool bValue) 
{
...
{
	HeadComponent->SetOwnerNoSee(!Value);
}

UObject* AALSBaseCharacter::LineTraceForObjectType(float Range, TEnumAsByte<EObjectTypeQuery> ObjectType)
{

	FVector StartLocation = GetMesh()->GetSocketLocation("FP_Camera"); // Starting point of the line trace
	FVector EndLocation = UKismetMathLibrary::GetForwardVector(GetControlRotation()) * Range + StartLocation; // Ending point of the line trace

	TArray<TEnumAsByte<EObjectTypeQuery>> ObjectTypesToTrace;
	ObjectTypesToTrace.Add(ObjectType);
	//ObjectTypesToTrace.Add(UEngineTypes::ConvertToObjectType(ECC_Pawn)); // Add object type to trace
	...
	if (bHit && EObjectTypeQuery::ObjectTypeQuery9 == ObjectType)
	{
		if (HitResult.GetComponent() && Cast<IALSInteractionInterface>(HitResult.GetComponent())) return HitResult.GetComponent();
		else if (HitResult.GetActor() && Cast<IALSInteractionInterface>(HitResult.GetActor())) return HitResult.GetActor();
		else return nullptr;
	}
	else if (bHit) 
	{
