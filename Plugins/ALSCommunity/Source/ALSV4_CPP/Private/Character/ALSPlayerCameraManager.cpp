...
bool AALSPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
	...
	const FVector& ADS = castedNewCharacter->GetFirstPersonCameraTargetADS();
	...
	if (castedNewCharacter->GetMovementState() == EALSMovementState::Stationary && castedNewCharacter->GetRecenterCamera()) {
		...
		if ((castedNewCharacter->GetRotationMode() == EALSRotationMode::Aiming && castedNewCharacter->GetStationaryRole() == EALSStationaryRole::Gunner_Standing)
			...
			const FRotator& ADSRotation =  castedNewCharacter->GetFirstPersonCameraRotationADS();

			const FRotator& InterpResult = FMath::RInterpTo(GetCameraRotation(), ADSRotation, DeltaTime,
				castedNewCharacter->StationaryTargetCameraRotationRate);
			...
			castedNewCharacter->GetController()->SetControlRotation(ADSRotation);
		}
		else {
			const FRotator& StationaryTargetRotation = FRotator(castedNewCharacter->GetCapsuleComponent()->GetComponentRotation());


			const FRotator& InterpResult = FMath::RInterpTo(GetCameraRotation(), StationaryTargetRotation, DeltaTime,
				castedNewCharacter->StationaryTargetCameraRotationRate);

			TargetCameraRotation = UKismetMathLibrary::RLerp(InterpResult, DebugViewRotation, GetCameraBehaviorParam(TEXT("Override_Debug")), true);
			castedNewCharacter->GetController()->SetControlRotation(StationaryTargetRotation);
		}
	}
	else {
	...
	if (castedNewCharacter->GetUsingADS() || castedNewCharacter->GetUseStationaryADS()) {
		...
		const FTransform& MixedTransform = UKismetMathLibrary::TLerp(TargetCameraTransform, ADSTransform,
			GetCameraBehaviorParam(
				NAME_Weight_FirstPerson));

		const FTransform& TargetTransform = UKismetMathLibrary::TLerp(MixedTransform, FTransform(DebugViewRotation,
			TargetCameraLocation,FVector::OneVector),GetCameraBehaviorParam(NAME_Override_Debug));
...
void AALSPlayerCameraManager::UpdateDamageEffect(float HPRatio)
{
	if (HPRatio > 0.33f) 
	{
		DesaturationAmount = 0.0f;
	}
	else if (HPRatio <= 0.33f && HPRatio >= 0.1f) 
	{
		DesaturationAmount = (HPRatio <= 0.33f) ? ((-30.0f * HPRatio) / 7.0f) + (10.0f / 7.0f) : 0.0f;
	}
	else 
	{
	...
	{
		VignetteStrength = ((-45.0f * HPRatio) / 7.0f) + (15.0f / 7.0f);
	}
	else
	{
		VignetteStrength = 1.5f;
	}
	//UKismetSystemLibrary::PrintString(GetWorld(), FString::Printf(TEXT("HP: %.2f | DesaturationWeight: %.2f | VignetteBlack: %.2f"), HPRatio, DesaturationAmount, VignetteStrength));
	ApplyValuesCameraModifier(HPRatio);
}