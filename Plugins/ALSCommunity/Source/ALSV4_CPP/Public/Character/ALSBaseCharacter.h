...
DECLARE_DELEGATE(OnExitDelegate)
...
class ALSV4_CPP_API AALSBaseCharacter : public ACharacter, public IALSInputInterface
{
	GENERATED_BODY()
...
public:
	...
	OnExitDelegate ExitDelegate;

	OnGunnerWeaponChanged GunnerWeaponChangedDelegate;
	...
	void SetOwnerHeadVisibility(bool Value);

	//Interaction system

	UPROPERTY(Replicated, BlueprintReadWrite, Category = "ALS|Interaction")
	TObjectPtr<AActor> ControlledInputInterceptor = nullptr;

	UPROPERTY(Replicated, EditAnywhere, BlueprintReadWrite, Category = "ALS|Component")
	AActor* ActiveWeaponActor = nullptr;

	UPROPERTY(BlueprintReadWrite, Category = "ALS|Interaction")
	TObjectPtr<UObject> CloseInteractable = nullptr;

	UPROPERTY(Replicated, BlueprintReadWrite, Category = "ALS|Interaction")
	EALSStationaryRole StationaryRole = EALSStationaryRole::None;

	UPROPERTY(BlueprintReadWrite, Category = "ALS|Interaction")
	UClass* ALSBaseAnimClass = nullptr;

	UPROPERTY(BlueprintReadWrite, Category = "ALS|Interaction")
	UClass* StationaryAnimClass = nullptr;

	UFUNCTION(Server, Reliable, BlueprintCallable, Category = "ALS|Interaction")
	void Server_SitCharacterDown(USceneComponent* SeatComponent);

	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "ALS|Interaction")
	void Multicast_SitCharacterDown(USceneComponent* SeatComponent);

	UFUNCTION(Server, Reliable, BlueprintCallable, Category = "ALS|Interaction")
	void Server_StandCharacterUp(FVector ExitLocation);

	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "ALS|Interaction")
	void MultiCast_StandCharacterUp(FVector ExitLocation);

	void HandleExitControlledInterceptor();
	...
	EALSStationaryRole GetStationaryRole() const { return StationaryRole; }
	...
	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "ALS|Character States")
	void Multicast_SetOverlayState(EALSOverlayState NewState, bool bForce = false);
	...
	UPROPERTY(Replicated,VisibleAnywhere, Category = "ALS|ADS")
	bool bUseStationaryADS = false;

	UFUNCTION(Client, Reliable)
	void Client_SetUseStationaryADS(bool value);

	bool GetUseStationaryADS();

	UPROPERTY(Replicated, VisibleAnywhere, Category = "ALS|ADS")
	bool bUseMainCameraTransform = false;
	...
	UFUNCTION(Client, Reliable)
	void NotifyOpenHud(bool bValue);
	...
	UFUNCTION(BlueprintCallable, Category = "ALS|Camera System")
	bool GetRecenterCamera();

	UFUNCTION(Client, Reliable)
	void Client_SetRecenterCamera(bool value);

	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "ALS|Camera System")
	FRotator StationaryTargetCameraRotation;

	UFUNCTION(BlueprintCallable, Category = "ALS|Camera System")
	FRotator GetStationaryTargetCameraRotation();

	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "ALS|Camera")
	float StationaryTargetCameraRotationRate=16.0f;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "ALS|IK")
	bool UsingHandIK = false;
	...
	void Server_ForwardMovementAction(float Value);

	UFUNCTION(Server, Reliable)
	void Server_RightMovementAction(float Value);

	UFUNCTION(Server, Reliable)
	void Server_ForwardMovementActionCompleted();

	UFUNCTION(Server, Reliable)
	void Server_RightMovementActionCompleted();

	UFUNCTION(Server, Reliable)
	void Server_CameraUpAction(float Value);

	UFUNCTION(Server, Reliable)
	void Server_CameraRightAction(float Value);

	UFUNCTION(Server, Reliable)
	void Server_JumpAction(bool bValue);
	...
	virtual void ForwardMovementActionCompleted_Implementation() override;

	virtual void RightMovementActionCompleted_Implementation() override;
	...
	virtual void CameraActionCompleted_Implementation() override;

	virtual void StanceAction_Implementation() override;

	virtual void WalkAction_Implementation() override;

	virtual void RagdollAction_Implementation() override;

	virtual void ScrollAction_Implementation(bool Value) override;
...
protected:
	...
	void UpdateCrawlRotation(float DeltaTime);

	bool CheckCanCrawl(bool ForTick);

	void CrawlDoEveryFrame(float TargetCapsuleSize, float CapsuleInterpSpeed, bool OnlyCapsuleUpdate, float DeltaTime);

	void CollisionDetectTraceStartEnd(FVector& Start, FVector& End, FVector& AxisValue);

	void GroundDetectTraceStartEnd(FVector& Start, FVector& End, FVector& AxisValue);

	float CalculateUpGroundAngle(FVector Axis, float Scale);

	UPROPERTY(VisibleAnywhere, BlueprintReadWrite, Category = "ALS|Crawl")
	float GroundAngle;
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, Category = "ALS|Input", BlueprintReadWrite)
	...
	UPROPERTY(EditDefaultsOnly, Category = "ALS|Input", BlueprintReadOnly)
	float StanceHoldTimeout = 0.9f;

	UPROPERTY(EditDefaultsOnly, Category = "ALS|Input", BlueprintReadOnly)
	bool bBlockCameraRotation = false;
	...
	UObject* LineTraceForObjectType(float Range, TEnumAsByte<EObjectTypeQuery> ObjectType);
...
private:
	...
	FTimerHandle CameraActionCompletedTimer;

	void EnableRecenterCamera();

	FTimerHandle CharacterAttachmentTimer;

	void AttachCharacter(USceneComponent* SeatComponent);

	UFUNCTION(NetMulticast, Reliable)
	void Multicast_AttachCharacter(USceneComponent* SeatComponent);
};
